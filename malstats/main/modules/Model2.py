...
class UserScoresPredictor():
    ...
    def __init__(self, user_name, model, site="MAL", shows_to_take="all"):
        ...
        self.user = User(user_name, scores=self.user_row.select(
            self.data.relevant_shows).row(0, named=True),
            scored_amount=self.user_row["Scored Shows"][0])
    ...
    def get_user_db(self, user, shows_to_take):
        # move to affinitydb
    ...
    def fetch_predictions(self):
        ...
        mean_scores_of_predictions = [self.tags.get_max_score_of_show(
            show, scores=self.data.mean_score_per_show) for show in predictions_per_show.keys()]
...
class ModelTester:
    ...
    def test_all(self, starting_model_index=1):
        ...
        for username in test_usernames:
            ...
            # user_shows_df_with_name = self.get_user_db(user, shows_to_take="all")
