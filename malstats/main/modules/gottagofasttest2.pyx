...
@cython.wraparound(False)
@cython.nonecheck(False)
cpdef double compute_affinity(np.ndarray[unsigned char] list1, np.ndarray[unsigned char] list2, int list_len):
...
@cython.wraparound(False)
@cython.nonecheck(False)
cpdef int count_common_shows(np.ndarray[unsigned char] list1, np.ndarray[unsigned char] list2, int list_len):
    cdef int count = 0
    for i in range(list_len):
